...
The official DeepSeek API is OpenAI-compatible at:

```bash
https://api.deepseek.com
```

However, the current Ritual Persistent Agent payload documented in `ritual-dapp-skills` does not include a custom OpenAI-compatible base URL field. The 26-field payload includes `provider`, `model`, and `llmApiKeyRef`, but no `baseUrl` / `apiBase` / custom endpoint argument. The latest persistent-agent example also only accepts first-party provider keys such as `OPENAI_API_KEY`; it does not document `OPENAI_BASE_URL`, `OPENAI_API_BASE`, or a DeepSeek endpoint override.

Because of that, official DeepSeek direct API cannot be documented as supported by the current Persistent Agent provider enum. Do not use this config yet:
...
If Ritual adds custom endpoint support later, the intended shape above is the right direction: provider `1` for OpenAI-compatible chat, `deepseek-chat` as the official DeepSeek model id, and `DEEPSEEK_API_KEY` only as a secret reference name. Until then, `PERSISTENT_AGENT_LLM_BASE_URL` is not validated or used by the adapter.

If using official DeepSeek through Persistent Agent becomes supported later, the raw `DEEPSEEK_API_KEY` must only exist inside `PERSISTENT_AGENT_ENCRYPTED_SECRETS`. `PERSISTENT_AGENT_LLM_API_KEY_REF=DEEPSEEK_API_KEY` would be only the key name the executor looks up after decrypting the bundle, not the raw key.

The encrypted secrets bundle would also need the selected DA credential. For example, with Hugging Face DA, the decrypted JSON shape would be:
...
1. Use OpenRouter provider enum `4` with an OpenRouter key and OpenRouter-routable model id.
2. Use a separate backend DeepSeek chat path outside Persistent Agent, with clear product labeling and real transaction semantics handled separately.
3. Wait for or add native DeepSeek/custom endpoint provider support in the Ritual Persistent Agent executor.

Do not submit placeholder keys, placeholder DA tokens, or untested model identifiers.
